...
O checkpoint local do Bloco 47 foi criado em `7791d11` com a mensagem `chore: valida flyway docker local ate bloco 47`, remote vazio e sem push. O Bloco 48 criou `scripts/local/validar-auth-rbac-csrf-local.ps1` e validou login admin local, cookie `HttpOnly`/`SameSite=Lax`, logout, bloqueio sem sessao, RBAC `ADMIN`/`MODERADOR`, fallback `/api/**`, CORS local e status de CSRF com resultado `OK_AUTH_RBAC_CSRF_LOCAL`.
...
O checkpoint local do Bloco 59 foi criado em `3111afc` com a mensagem `style: transplanta cards detalhe visual ate bloco 59`, remote vazio e sem push. O Bloco 60 usa a logo real local em `/logo.webp` no header publico e aproxima visualmente o wizard `/anunciar` da referencia do clone de producao, sem alterar fluxo funcional, backend, banco, upload, pagamento, Pix/Efi ou API externa.
...
O checkpoint local do Bloco 32.1 foi criado em `f6189f0` com a mensagem `test: valida render publico sintetico ate bloco 32.1`, remote vazio e sem push. O Bloco 33 validou `/anunciar` em desktop/mobile com dados sinteticos, PostgreSQL descartavel, backend/frontend locais e prints em `docs/v3/evidencias/bloco-33/prints/`. O wizard permanece sem upload real, pagamento, Pix/Efi, Premium obrigatorio, e-mail real, WhatsApp real, autopublicacao, stores, scroll lock ou elemento mobile flutuante.

O Bloco 33.1 corrige apenas textos publicos sem acento no wizard `/anunciar`, como `Revisão final`, `Solicitação recebida`, `Confirmações`, `Etapa concluída` e `botão final`. Nenhuma regra de negocio, contrato, DTO, rota, validacao, seguranca ou fluxo real foi alterado.

O checkpoint local dos Blocos 33/33.1 foi criado em `b7f5f98` com a mensagem `test: valida wizard anunciar sintetico ate bloco 33.1`, remote vazio e sem push. O Bloco 34 reorienta o escopo para paridade do wizard `/anunciar` com a producao observavel: a consulta publica a `https://topsdojob.com/anunciar` redirecionou para `/?next=/anunciar` com age gate, sem clicar em `Aceitar`; a V3 local recebeu ajustes leves de CTA/confianca para `PUBLICAR SEU ANUNCIO`, mantendo stores, upload real, pagamento, Pix/Efi real, e-mail real, WhatsApp real e publicacao automatica fora do fluxo.
...
.\scripts\local\validar-flyway-real-local.ps1
