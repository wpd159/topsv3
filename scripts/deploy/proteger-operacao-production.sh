...
op_smoke() {
  ...
  while [ "${SECONDS}" -lt "${readiness_deadline}" ]; do
    [ "${OP_CANCEL_RC:-0}" -eq 0 ] || return "${OP_CANCEL_RC}"
    if _op_http_probe http://127.0.0.1:28080/api/health/readiness readiness "deploy-${OP_ID}-boot"; then
      ready=1
      break
    fi
    sleep 2
  ...
  for attempt in 1 2 3 4; do
    ...
    if _op_verify_runtime "${sha}" "${manifest}" &&
      _op_http_probe http://127.0.0.1:28080/api/health/readiness readiness "${correlation}-ready" &&
...
op_begin() {
  ...
  OP_CANCEL_RC=0 OP_ORIGINAL_RC=0 OP_RECOVERING=0 OP_MIGRATION_STARTED=0 OP_RECONCILE_ONLY=0
  OP_ACTIVE=1
  _op_install_traps
  _op_state || return 1
...
_op_manual() {
  local mode="$1" root="$2" id="$3" expected="${4:-}" confirmation="${5:-}" state old_pid old_start old_boot
  _op_id "${id}" || return 2
