...
// Input nativo, sem componente compartilhado. Foco usa apenas border-color +
// box-shadow estatico (sem transition-all, sem animacao pesada).
...
// Teste controlado: mesmo RegisterForm, mas em 2 etapas. Somente a etapa
// atual existe no DOM (currentStep === 1 / === 2, sem display:none,
// visibility:hidden, opacity, max-height, accordion, transform ou
// animacao/transition-all). Estado dos campos fica neste componente pai e e
// preservado ao avancar/voltar. Nenhum fetch juridico no mount: os 3
// documentos so sao buscados no submit (fallback local ja cobre falha).
export function RegisterFormSteps({
  ...
  const step1Valid = emailValid && !erros.email && passwordOk && credenciaisConferem
  const step2Valid =
  ...
  const allFieldsValid = step1Valid && step2Valid
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      // Documentos juridicos carregados aqui (so no submit), nao no mount.
      const [termosDocumento, privacidadeDocumento, promocionalDocumento] = await Promise.all([
      ...
      const documentosJuridicos: DocumentosJuridicosState = {
    ...
    <div className={cn('w-full', className)}>
      <div className="flex flex-col items-center gap-2 text-center">
        ...
        <p className="text-xs font-semibold text-gray-400">
          {currentStep === 1 ? 'Etapa 1 de 2' : 'Etapa 2 de 2'}
        </p>
      </div>
      ...
      {currentStep === 1 && (
        <div className="mt-4 space-y-4">
          ...
          <Button
            ...
            disabled={!step1Valid}
            className={`w-full py-5 mt-2 font-semibold text-white ${
              step1Valid ? 'bg-[#FC1EAD] hover:bg-[#e01a9a]' : 'bg-gray-300 cursor-not-allowed'
            }`}
          >
            {getStep1ButtonLabel()}
          </Button>
      ...
      {currentStep === 2 && (
        <div className="mt-4 space-y-4">
          <button
            type="button"
            onClick={handleBack}
            className="text-sm text-[#FC1EAD] font-medium hover:underline"
          >
            Voltar
          </button>
          ...
          <div className="space-y-2 mt-4 text-xs">
            ...
            <label className="flex items-center gap-1">
              <Checkbox
                ...
                onCheckedChange={(v) => setTerms((t) => ({ ...t, privacidade: !!v }))}
              />
