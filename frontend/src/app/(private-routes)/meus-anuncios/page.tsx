...
import { MeusStoriesPanel } from '@/components/stories/meus-stories-panel'
import { StoryCreateDialog } from '@/components/stories/story-create-dialog'
import type { MinhaContaStory } from '@/lib/minha-conta-stories-api'
import {
  listarMeusAnuncios,
...
export default function MeusAnunciosPage() {
  ...
  const [storyRefreshVersion, setStoryRefreshVersion] = useState(0)
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      if (error instanceof MeusAnunciosApiError && error.status === 401) {
        setErro({ titulo: 'Sessão necessária', mensagem: 'Entre novamente para consultar seus anúncios.' })
      } else if (error instanceof MeusAnunciosApiError && error.status === 403) {
        setErro({ titulo: 'Acesso negado', mensagem: 'Sua conta não possui acesso a esta listagem.' })
      } else if (error instanceof MeusAnunciosApiError && error.status === 404) {
        setErro({ titulo: 'Integração indisponível', mensagem: 'A consulta de anúncios não está disponível neste ambiente.' })
      } else {
        setErro({ titulo: 'Não foi possível carregar seus anúncios.', mensagem: 'Não foi possível concluir a consulta. Tente novamente em instantes.' })
      }
    } finally {
      setLoading(false)
    }
  ...
  const aplicarCicloVida = useCallback((resultado: MeuAnuncioCicloVida, acao: CicloVidaAcao) => {
    setAnuncios((atuais) => {
  ...
  const aplicarStory = useCallback((story: MinhaContaStory) => {
    if (story.modoConteudo === 'ANUNCIO' && story.anuncioId) {
      setAnuncios((atuais) => atuais.map((anuncio) => anuncio.id === story.anuncioId
        ? { ...anuncio, storyAtivo: { ...story, anuncioId: story.anuncioId as string } }
        : anuncio))
    }
    setStoryRefreshVersion((value) => value + 1)
  }, [])
  ...
  const storyGlobalDisabled = loading || Boolean(erro)
    ...
    <PainelShell title="Meus anúncios" description="Consulte o status, a localização e a capa pública dos seus anúncios.">
      <div className="space-y-6">
        <div className="flex flex-col justify-end gap-2 sm:flex-row sm:items-center">
          ...
          {storyGlobalDisabled ? <span id="story-global-indisponivel" className="sr-only">{storyGlobalReason}</span> : null}
          <Link href="/anunciar/wizard" className="inline-flex min-h-11 w-full items-center justify-center rounded-lg bg-[#FC1EAD] px-5 py-2.5 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-[#e01a9a] hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FC1EAD] focus-visible:ring-offset-2 sm:w-auto">
        ...
        <MeusStoriesPanel
          refreshVersion={storyRefreshVersion}
          onChanged={() => {
            setStoryRefreshVersion((value) => value + 1)
            void carregar()
          }}
        ...
        {!loading && !erro && anuncios.length > 0 ? (
          <section className="rounded-[28px] border border-slate-200 bg-white px-5 py-4 shadow-sm">
            <p className="text-sm font-medium text-slate-600">{anuncios.length} {anuncios.length === 1 ? 'anúncio encontrado' : 'anúncios encontrados'}</p>
          </section>
        ) : null}

        {loading ? (
          <div className="flex min-h-[240px] items-center justify-center rounded-[28px] border border-slate-200 bg-white text-sm text-slate-500 shadow-sm">Carregando seus anúncios...</div>
        ) : erro ? (
          <div className="flex min-h-[240px] flex-col items-center justify-center rounded-[28px] border border-rose-200 bg-rose-50 px-6 text-center shadow-sm">
            ...
            <button type="button" onClick={() => void carregar()} className="mt-5 rounded-lg bg-[#FC1EAD] px-5 py-2.5 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-[#e01a9a] hover:shadow-sm">Tentar novamente</button>
          </div>
        ) : anuncios.length === 0 ? (
          <div className="flex min-h-[260px] flex-col items-center justify-center rounded-[28px] border border-dashed border-slate-300 bg-white px-6 text-center shadow-sm">
            ...
            <p className="mt-2 max-w-md text-sm leading-6 text-slate-500">Você já pode enviar uma mídia aos Stories. Para promover um anúncio, publique-o primeiro.</p>
          </div>
        ) : (
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
            {anuncios.map((anuncio) => (
              <MeuAnuncioCard key={anuncio.id} anuncio={anuncio} onCicloVida={aplicarCicloVida} onStoryOpen={abrirStory} />
            ))}
          </div>
        )}
        ...
        <StoryCreateDialog
          ...
          anuncios={anuncios}
          initialAnuncioId={storyTargetId}
          returnFocusTo={storyReturnFocusRef.current}
          onSuccess={aplicarStory}
        />
      </div>
    </PainelShell>
  )
