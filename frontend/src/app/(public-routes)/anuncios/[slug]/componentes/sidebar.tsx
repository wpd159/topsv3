...
const WhatsAppIcon = (props: any) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" {...props}>
    <path
      fill="#25D366"
      d="M256 0C114.6 0 0 114.6 0 256c0 45.3 11.7 89.2 33.9 128.2L0 512l130.8-33.2C168.4 496.4 211.6 512 256 512c141.4 0 256-114.6 256-256S397.4 0 256 0z"
    />
    <path
      fill="#FFF"
      d="M391.1 358.3c-5.9 16.5-28.3 30.8-46.5 34.9-12.3 2.6-28.3 4.6-82.3-17.6-69.3-28.7-114.3-99.3-117.8-103.9-3.4-4.6-28.1-37.4-28.1-71.4 0-34 17.6-50.8 24-57.8 5.9-6.5 13-8.1 17.6-8.1 4.6 0 8.8 0.1 12.6 0.2 4 0.1 9.4-.1 14.1 10.8 5.4 12.9 18.2 44.5 19.8 47.8 1.6 3.4 2.7 7.4 0.5 11.8-2.2 4.3-3.3 7-6.5 10.8-3.1 3.7-6.9 8.3-9.9 11.2-3.3 3.1-6.7 6.5-2.9 12.9 3.8 6.5 17 27.9 36.4 45.2 25.1 22.5 45.2 29.5 52.2 32.7 7 3.2 11.1 2.7 15.3-1.6 4.2-4.3 17.7-20.7 22.5-27.8 4.8-7 9.5-5.9 15.9-3.8 6.5 2.2 41.2 19.4 48.2 23 7 3.5 11.7 5.3 13.4 8.1 1.7 2.7 1.7 16.1-4.2 32.5z"
    />
  </svg>
)
...
export default function Sidebar({ anuncio }: SidebarProps) {
  ...
  const formatarCategoria = (categoria?: string | null) => {
  ...
  const whatsappUrl = useMemo(() => {
    if (anuncio.restrictedPreview) return null
    const tel = (anuncio.telefone ?? '').trim()
    if (!tel) return null
  ...
  }, [anuncio.telefone, anuncio.restrictedPreview])
  ...
  const handleWhatsAppClick = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      const res = await fetch(`${API}/cliques-whatsapp/${encodeURIComponent(anuncio.slug)}`, {
        method: 'POST',
        credentials: 'include',
      })
  ...
  const handleChatClick = () => {
    if (anuncio.restrictedPreview) {
      anuncio.onRequestVerification?.()
      return
    }
    if (!anuncio.username) return
    router.push(`/chat?usuario=${encodeURIComponent(anuncio.username)}`)
  }
      ...
      <div className="space-y-6">
        <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
          ...
          <div className="space-y-1 text-sm text-gray-600">
            {anuncio.idade != null ? (
              <p className="text-sm font-medium text-gray-700">{anuncio.idade} anos</p>
            ) : null}

            <p>{formatarCategoria(anuncio.tipo)}</p>
            <p>{localizacaoLabel}</p>
            <p className="mt-2 font-semibold text-pink-600">{anuncio.valor}</p>
          </div>

          <div className="mt-5 flex flex-col gap-3">
            {anuncio.restrictedPreview && (
              <Button
                type="button"
                onClick={anuncio.onRequestVerification}
                className="w-full bg-[#FC1EAD] py-3 font-semibold text-white hover:bg-[#e01a9a]"
              >
                {anuncio.requiresStrongVerification ? 'Verificação forte 18+' : 'Verificação 18+'}
              </Button>
            )}
            ...
            {whatsappUrl ? (
              <Button
                variant="outline"
                className="flex w-full items-center justify-center py-3 font-semibold"
                onClick={handleWhatsAppClick}
              >
                <span className="inline-flex items-center">
                  <WhatsAppIcon className="mr-2 h-5 w-5" /> Conversar no WhatsApp
                </span>
              </Button>
            ) : (
              <Button
          ...
          <button
            className="mx-auto mt-3 flex items-center gap-1 text-xs text-gray-400 transition-all hover:text-gray-600"
