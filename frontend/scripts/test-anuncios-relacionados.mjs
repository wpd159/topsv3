...
assert.match(related, /useEffect/)
assert.match(related, /scrollWidth - container\.clientWidth/)
assert.match(related, /container\.scrollBy/)
assert.match(related, /behavior: 'smooth'/)
assert.match(related, /aria-label="Ver cards anteriores"/)
assert.match(related, /aria-label="Ver mais cards"/)
assert.match(related, /sm:hidden/)
assert.match(related, /onScroll=\{updateScrollControls\}/)
assert.match(related, /sm:grid/)
assert.match(related, /w-\[190px\]/)
assert.doesNotMatch(related, /<section[^>]*\bhidden\b/)
assert.doesNotMatch(related, /Math\.random|Chat|WhatsApp|Favorit|evento_visualizacao|registrarVisualizacao/)
