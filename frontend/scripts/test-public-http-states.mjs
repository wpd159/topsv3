...
for (const scope of geographicScopes) {
  await test(`${scope.path}: ausente e page=0/1/2 preservam API, canonical e navegacao zero-based sem redirect`, async () => {
    for (const pageValue of [undefined, "0", "1", "2"]) {
      ...
      const previousLinks = elementNodes(navigation, (node) => node.props?.children === "Anterior")
      assert.equal(previousLinks.length, previous.length)
      if (previous.length) assert.equal(previous[0].props.href, new URL(previousLinks[0].props.href, "https://topsdojob.com").toString())
      ...
      assert.equal(next[0].props.href, nextUrl.toString())
