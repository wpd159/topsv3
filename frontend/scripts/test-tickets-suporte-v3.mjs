...
assert.ok(userPage.includes('Acompanhe a resposta em Meus tickets.'), 'Sucesso deve orientar o acompanhamento pelo nome real da tela.')
assert.doesNotMatch(header, /Meus Suportes/, 'Menu deve usar o mesmo nome da tela de tickets.')
assert.equal((header.match(/> Meus tickets/g) ?? []).length, 2, 'Menus desktop e mobile devem identificar Meus tickets.')
assert.ok(header.includes('router.push("/meus-tickets")') && header.includes('go("/meus-tickets")'), 'Destinos dos menus de suporte devem permanecer inalterados.')
