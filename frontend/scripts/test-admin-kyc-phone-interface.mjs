...
assert.ok(userDetail.includes('AdminKycReviewGrid'), 'Usuarios deve usar a revisao compartilhada.')
assert.ok(userDetail.includes('AdminKycUploadDialog'), 'A gestao do usuario deve concentrar inclusao e substituicao.')
assert.ok(adDocuments.includes('AdminKycReviewGrid'), 'Anuncios deve usar a mesma revisao compartilhada.')
...
assert.ok(reviewGrid.includes('decideAdminKycSubmission') && reviewGrid.includes('confirmDecision'), 'As duas telas devem usar a decisao KYC canonica compartilhada.')
assert.ok(reviewGrid.includes('Todos os documentos deste envio serão marcados como validados.'), 'A confirmacao deve explicitar o escopo da validacao.')
assert.ok(userDetail.includes("permissoes.includes('DOCUMENTO_REVISAR')"), 'A tela do usuario deve falhar fechada sem permissao documental.')
assert.ok(userDetail.includes("role === 'ADMIN' || role === 'MODERADOR'"), 'ADMIN e MODERADOR devem reutilizar o RBAC canonico.')
assert.ok(userDetail.includes('detail.kycEnvios') && userDetail.includes('onDecisionComplete={load}'), 'A gestao deve funcionar pelo usuario e atualizar os estados sem depender de anuncio.')
