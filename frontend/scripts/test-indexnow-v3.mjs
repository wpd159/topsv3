...
assert.match(productionWorkflowSource, /Build and activate production release/)
assert.match(productionWorkflowSource, /op_expect_indexnow "\$\{indexnow_key\}"/)
assert.match(productionWorkflowSource, /op_run mutating docker run --rm -i --pull never/)
assert.ok(productionWorkflowSource.indexOf('op_begin "${deploy_root}" "${release_sha}"')
  < productionWorkflowSource.indexOf('op_expect_indexnow "${indexnow_key}"'))
...
assert.match(productionWorkflowSource, /awk "!\/\^INDEXNOW_KEY=\/" "\$\{env_file\}" > "\$\{temporary\}"/)
assert.match(productionWorkflowSource, /printf "INDEXNOW_KEY=%s\\n" "\$\{key\}" >> "\$\{temporary\}"/)
assert.match(productionWorkflowSource, /chmod "\$\{mode\}" "\$\{temporary\}"/)
assert.match(productionWorkflowSource, /chown "\$\{owner\}" "\$\{temporary\}"/)
assert.match(productionWorkflowSource, /mv -f -- "\$\{temporary\}" "\$\{env_file\}"/)
