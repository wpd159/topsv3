...
assert.match(sources.video, /preload=\{preload\}/)
...
const loadIndex = sources.modal.indexOf("const created = await loadChallenge()", policyIndex)
const helperIndex = sources.modal.indexOf("const loadChallenge = useCallback(")
const challengeIndex = sources.modal.indexOf("const challengeKey =", helperIndex)
const createIndex = sources.modal.indexOf("return createVisitorChallenge(", challengeIndex)
assert.ok(statusIndex >= 0)
assert.ok(policyIndex > statusIndex)
assert.ok(loadIndex > policyIndex)
assert.ok(helperIndex >= 0)
assert.ok(challengeIndex > helperIndex)
