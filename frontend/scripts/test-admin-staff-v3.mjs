...
const assertions = [
  ...
  ['exclusao logica no adapter', api.includes('deleteAdminStaff') && api.includes("method: 'DELETE'")],
  ['confirmacao explicita', table.includes('<DialogTitle>Excluir este staff?</DialogTitle>') && table.includes('O acesso administrativo ser&aacute; removido. O hist&oacute;rico de auditoria ser&aacute; preservado.') && table.includes("{removing ? 'Excluindo...' : 'Excluir staff'}")],
  ['somente staff ativo pode ser excluido', table.includes('canDelete && staff.ativo')],
  ['duplo clique bloqueado na exclusao', table.includes('if (!target || removing) return') && table.includes('disabled={removing}')],
  ['endpoint canonico', controller.includes('@DeleteMapping("/{id}")') && controller.includes('service.remover(')],
  ['auditoria e sessoes preservadas', service.includes('"STAFF_REMOVER"') && service.includes('invalidarSessoesAposCommit(id)')],
  ['sem exclusao fisica da conta', !service.includes('usuarios.delete') && !service.includes('usuarioRepository.delete')],
  ['openapi da exclusao logica', openapi.includes('operationId: deleteAdminStaff') && openapi.includes('preserva conta, papel e auditoria')],
