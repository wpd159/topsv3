...
No terceiro CI (`35550832594`, head `d70a412e`), Ctrl/Meta abriu a área corretamente; a espera pela abertura seguinte, com Shift, expirou. A causa dessa ocorrência continua inconclusiva. A mesma navegação passou em Windows e Chromium Linux isolado. Um controle local demonstrou outra lacuna do harness: `Promise.all` pode ocultar o erro de estabilidade do clique quando a espera pelo evento da página vence primeiro. A coleta passa a preservar ambos os resultados/erros, inclusive quando não há popup, sem mudar gestos, assertions ou os 5 segundos. O controle negativo continua reprovando. Essa correção é de diagnóstico, não uma alegação de correção da causa não observada no CI; a candidata final precisa de sua própria execução, e uma nova falha deve ser apurada pelas evidências preservadas.
