...
- A remocao parte de `origin/main=6d3991f61c6c24cc5a7092b84fba699489b3ae17`, na branch `codex/remove-rejected-deploy`. Por decisao do proprietario, o Pipeline Hermetico V2 e os componentes exclusivos do controlador/candidata rejeitado deixam a arvore ativa; suas instrucoes anteriores nao sao mais operacionais e o historico Git permanece preservado.
- O mecanismo mantido e `.github/workflows/deploy-production.yml`, exclusivamente manual, com ref explicita e identificacao por SHA, empacotamento, transferencia, build, health/readiness, smoke, troca atomica de symlink e rollback. Backup, Flyway e ferramentas compartilhadas de dados permanecem preservados.
- A release publicada `4fac7d9034fba38180043a261d96f3c94865b4af` e apenas referencia do mecanismo comprovado. A main preserva suas diferencas intencionais e nao e certificada como equivalente a producao.
- O hotfix P0.1 e as migrations, inclusive V053, permanecem intactos. O gate SEO permanece `blocked` no CI e `public` depois do build de producao e no estagio Docker que produz a imagem frontend.
- Esta tarefa nao acessa producao e nao executa deploy, backfill ou migration externa. Push, PR e merge nao iniciam publicacao; deploy futuro continua sujeito a autorizacao especifica.
