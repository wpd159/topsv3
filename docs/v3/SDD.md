...
- A base canonica desta correcao e `origin/main=66696d5f11dd5a322b9b0850c7a4cf717fe3ff44`; o PR #24 e seu CI pos-merge `33324582326` permanecem como evidencia da Fase 1.
...
- O `verify` `33350467422` certificou o artifact `9743533209`. No `candidate` `33354579527`, a validacao local passou, mas o job criou um segundo TAR e executou `candidate-remote runtime/artifact` a partir de `runtime`; o resolvedor executado com essa workdir procurou efetivamente `runtime/runtime/artifact`, deslocando o manifesto certificado.
- O layout V2 adota uma unica unidade `candidate-payload.tar`, criada uma vez no build-once sob `candidate-root/`, com SHA e tamanho em certificacao externa. O candidate valida e envia exatamente esse arquivo, compara SHA/tamanho local e remoto, valida a tabela TAR, extrai com seguranca e usa o mesmo resolvedor/manifest-verify local e remoto.
- O laboratorio SSH real e descartavel aprovou 10/10 transportes byte a byte, 15/15 casos negativos, diagnostico antes do cleanup e zero residuos. O round-trip pequeno do GitHub permanece gate obrigatorio e nao constroi a aplicacao.
- O artifact e o manifesto de `33350467422` nao podem ser reutilizados depois dessa alteracao. A candidata real ainda nao foi repetida; depende de novo `verify`, novo artifact certificado e nova autorizacao. `switch` permanece bloqueado.
...
- O runner operacional de fixtures separa quatro acoes explicitas: `app.fixture.admin-provision.enabled=true` provisiona ADMIN por `stdin`; `app.fixture.stories.enabled=true` reconcilia dados sem ler credencial; `app.fixture.owner-credential.enabled=true` atualiza por `stdin` apenas a credencial BCrypt do proprietario canonico; `app.fixture.auth-smoke.enabled=true` reconcilia isoladamente tres contas `USUARIO` sinteticas para smoke de autenticacao. Todas permanecem restritas ao profile `homologacao`, desabilitadas no startup normal e recusadas fora desse ambiente.
...
- A V021 cria a fonte canonica `categoria_home`, pois a auditoria confirmou que nenhuma entidade ou tabela V3 equivalente existia. A V027 restringe o vinculo as cinco categorias canonicas ja oferecidas pelo wizard e deriva no backend o destino `/anuncios` com o parametro `categoria={codigo}`; card sem vinculo ou destino livre deixa de ser valido.
- `GET /api/public/categorias-home` retorna somente cards ativos e ordenados, com URL publica resolvida pelo `ObjectStorage` e sem expor object key. `GET /api/public/anuncios` com o parametro `categoria={codigo}` lista somente anuncios publicados e aprovados cuja categoria persistida pelo wizard corresponde ao vinculo.
- O painel usa exclusivamente `/api/admin/categorias-home` para criar e editar vinculo, nome, descricao, ordem, status e imagem. A taxonomia vem do backend, a imagem continua no R2 canonico, a ordem e unica e dois cards ativos nao podem compartilhar categoria. A V028 cataloga `ADMIN_CONFIGURAR` e a vincula somente ao papel `ADMIN`, mantendo a autoridade granular exigida pelo controller sem bypass de HML.
- Home e listagem usam exclusivamente `frontend/src/lib/public-catalog-api.ts`; nao ha categoria, destino, imagem ou lista de anuncios simulada como fallback. As categorias editoriais da producao serao recriadas manualmente no painel V3, sem importador historico.
...
- A auditoria somente leitura de 2026-07-17 confirmou que as fotos publicas vigentes usam o bucket `topsdojob-fotos`, a origem `https://pub-567428d3703244d483815a05a1e0e0d9.r2.dev` e chaves `anuncios/fotos/original/{hex32}.{ext}`. HTML e Open Graph usam a mesma URL original; o JSON-LD observado na pagina de anuncio nao publica outra imagem.
- No cutover, a V3 pode derivar essas URLs preservadas sem mover ou renomear objetos, desde que bucket, origem, prefixo, MIME, classificacao `LIVRE` e formato da chave coincidam exatamente com a configuracao auditada. A base publica gerenciada do HML continua independente e vazia ate aprovacao propria.
- A reconciliacao final aplicou tambem o bloqueio canonico de titulo/slug generico ja existente no backend: os 115 registros de evidencia historica resultaram em 113 URLs runtime realmente indexaveis. Duas execucoes PostgreSQL 17 e a renderizacao SSR produziram 113 entradas, fingerprint `ba3630476d887aee0a229ce8959963ec`, zero origem publica invalida e zero URL restrita ou privada.
...
- O snapshot `kyc-private-89855f26df6a5dd4`, de `2026-07-16T18:03:27.812565Z`, comprovou que `usuarios/{identificador}/documentos/**` usa `usuarios.id`: as 1.168 referencias unicas analisadas coincidiram com o usuario da linha, sem vinculo incorreto ou referencia a pasta de outro usuario.
- A origem continha 1.924 linhas documentais, 1.168 referencias unicas, 756 repeticoes e 1.314 objetos no bucket privado. Caminhos aninhados abaixo de `documentos/` foram aceitos; 146 objetos sem referencia de banco permaneceram fora da importacao automatica.
- PDF unico e pares JPG/PNG frente/verso foram reconhecidos pelo binario real. Checksum repetido para o mesmo usuario foi consolidado; checksum entre usuarios bloqueia promocao. O resultado foi 1.008 documentos unicos elegiveis e 100 referencias duplicadas consolidadas.
- O destino usa exclusivamente `ObjectStorage`, area `PRIVATE_DOCUMENT` e prefixos isolados `hml/documentos/importacao/{ambiente}/sha256/`. Nao existe URL publica; a leitura administrativa temporaria foi validada sem registrar a URL assinada.
- Nos dois ambientes R2 independentes, a primeira execucao criou 1.008 objetos e a segunda criou zero. Os resultados normalizados produziram o mesmo fingerprint SHA-256 `c77bd893058953b8e5c962d05f1ccfe6465c7ff80f528ed7e434c9379146199a`.
- Nos dois PostgreSQL 17/V001-V026 descartaveis, a primeira reconciliacao criou 1.008 documentos e a segunda criou zero; ambos produziram o fingerprint SHA-256 `0bc16e953f00a49f5171dcb92ed9aab2d1308bcfd3f07192dbb7eec2ed1471a2`, com zero FK invalida, duplicacao, checksum entre usuarios ou documento fora do storage privado.
- O estado KYC legado nao usa `is_verificado`, que confirma conta/e-mail. Estados administrativos explicitos prevalecem; sem decisao explicita, o frontend de producao exibe `Conta verificada` quando `documentosUrls`/`totalDocumentos` possui ao menos um registro. O importador replica essa precedencia e traduz `APROVADO` para `VALIDADO`, `REPROVADO` para `REJEITADO` e os demais para `PENDENTE` somente nos documentos conciliados.
- No snapshot canonico `final-20260715T225154Z`, os 1.450 usuarios distintos resultaram em 589 `APROVADO`, tres `PENDENTE`, zero `REPROVADO` e 858 `NAO_INICIADO`. O usuario tecnico de origem `1054` comprova o fallback: quatro documentos, status administrativo nulo e nenhuma solicitacao de revisao; na prova isolada ele permaneceu `APROVADO` e seu documento conciliado ficou `VALIDADO`.
- A auditoria documental posterior observou 1.452 usuarios porque dois cadastros foram criados depois do snapshot canonico; nao houve duplicacao por join documental. Um usuario ja existente tambem recebeu documento depois do corte, portanto a fotografia viva nao substitui as contagens do snapshot.
- Duas reconciliacoes de status em PostgreSQL 17 produziram o mesmo fingerprint SHA-256 `4bdd0958142ee48a80f50217895835908fe03dbbd55ecc8aa0443c36bc6578c3`. Nenhum objeto foi retransmitido: as 1.008 copias privadas e o fingerprint R2 `c77bd893058953b8e5c962d05f1ccfe6465c7ff80f528ed7e434c9379146199a` permaneceram estaveis.
- Permanecem para analise manual 58 arquivos de tipo/conteudo invalido, dois conjuntos conflitantes e quatro pastas sem usuario. Nenhum usuario aprovado foi rebaixado ou recebeu exigencia de reenvio, e nenhuma duvida foi promovida automaticamente.
...
- O deploy HML passa a executar `mvn --batch-mode --no-transfer-progress verify` como gate obrigatorio antes de qualquer etapa remota. O mesmo workflow reconcilia, por flag e secret exclusivos do environment `hml`, contas sinteticas ativa/confirmada, ativa/pendente e desativada/confirmada; a acao usa o runner unico, nao cria anuncio, credito, Premium, Story, KYC, token ou sessao e nunca executa os importadores historicos.
