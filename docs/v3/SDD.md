...
- O fixture operacional unico de homologacao, `HmlStoriesFixtureService`, tambem prepara o catalogo publico ficticio necessario aos contratos e ao sitemap. Ele permanece restrito ao profile `homologacao`, depende do runner explicito `app.hml-admin-provision.enabled=true`, e sincroniza dados deterministas sem duplicar usuarios, anuncios, midias ou Stories em reexecucoes. A publicacao e a execucao no HML online dependem de etapa posterior expressa.
