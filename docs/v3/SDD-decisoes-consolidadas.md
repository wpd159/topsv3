...
- 2026-07-15 (KYC no dry-run): documento legado so entra no fluxo operacional quando usuario/vinculo, origem privada, existencia, tipo real, tamanho, checksum e parte documental estiverem comprovados. URL HTTP/HTTPS nunca e convertida em URL V3 e permanece em quarentena.
- O dry-run KYC baixa somente PDFs candidatos a `UNICO`; imagens sem frente/verso persistido ficam em quarentena sem inferencia por nome ou ordem. Documento elegivel entra como `PENDENTE`, nunca como aprovado por heranca duvidosa.
- O destino documental e exclusivamente o R2 privado HML no prefixo `hml/documentos/importacao/`, com chave por checksum, sem PII, URL publica, storage concorrente ou alteracao da origem.
