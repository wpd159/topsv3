...
class AdminArquivoStoryControllerSecurityTest {
  @Autowired private MockMvc mockMvc;
  @MockBean private AdminArquivoStoryService service;

  @Test
  void exigeAdminFinalidadeEPermissaoSeparadaParaExportacaoEMidia() throws Exception {
    ...
    var detalhe = new Detalhe(id, UUID.randomUUID(), null, UUID.randomUUID(),
        ...
        "ABRANGIDA", OffsetDateTime.now(), OffsetDateTime.now().plusDays(1),
        OffsetDateTime.now().plusYears(2), "LIMITE_STORY", List.of());
    ...
    mockMvc.perform(post("/api/admin/registros/stories")
            .param("finalidade", finalidade.name())
            .with(csrf())
            .with(authentication(token(PapelUsuario.ADMIN, true, false))))
        .andExpect(status().isOk())
        .andExpect(header().string("Cache-Control", "no-store"));
    mockMvc.perform(post("/api/admin/registros/stories/{id}", id)
            .param("finalidade", finalidade.name())
            .with(csrf())
            .with(authentication(token(PapelUsuario.ADMIN, true, false))))
        .andExpect(status().isOk());
    mockMvc.perform(post("/api/admin/registros/stories/{id}/exportacao", id)
            .param("finalidade", finalidade.name())
            .with(csrf())
            .with(authentication(token(PapelUsuario.ADMIN, true, true))))
        .andExpect(status().isOk())
        .andExpect(header().string("Cache-Control", "no-store"));
    mockMvc.perform(post("/api/admin/registros/stories/{id}/midias/{midiaId}/arquivo", id, midiaId)
            .param("finalidade", finalidade.name())
            .with(csrf())
            .with(authentication(token(PapelUsuario.ADMIN, true, true))))
        .andExpect(status().isOk());
  ...
  private UsernamePasswordAuthenticationToken token(
    ...
    if (leitura) authorities.add(new SimpleGrantedAuthority("ARQUIVO_PUBLICIDADE_LER"));
    if (exportacao) authorities.add(new SimpleGrantedAuthority("ARQUIVO_PUBLICIDADE_EXPORTAR"));
